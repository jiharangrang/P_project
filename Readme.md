...
- `raspbot/perception/` : ROI 계산, IPM(warp), HSV/그레이 임계 기반 라인 검출, 히스토그램/heading 분석, 디버그 시각화.
...
5) 에러/방향 추정:  
   - `compute_lane_error`: 도로(검정) 무게중심으로 중앙 오차(-1~1), 히스토그램(left/center/right) 통계.  
...
| `heading.smooth_alpha`, `heading.thresh` | `0.20`, `0.01` | heading EMA 가중치/적용 임계 |
...
- Heading: `heading_thresh_x100`, `heading_smooth_x100`.
...
- `raspbot/perception/lane_detection.py` : HSV+그레이 기반 라인 이진화, 히스토그램 분석, 중심 오차 및 heading 추정.
- `raspbot/perception/visualization.py` : 이진 디버그 뷰에 방향/히스토그램/조향/FPS/turn 임계/센터 포인트 표시.
